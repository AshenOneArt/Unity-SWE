                ...
                out LightLoopOutput lightLoopOutput)
{
    ...
    //将采样Shadow挪至waterBSDF当中
    // First of all we compute the shadow value of the directional light to reduce the VGPR pressure

    float dither = R2_dither(posInput.positionSS);    
    waterLightLoopData.StartPos = posInput.positionWS;
    waterLightLoopData.EndPos = ScreenToWorldPosition(posInput.positionNDC.xy) - _WorldSpaceCameraPos.xyz;
    waterLightLoopData.RayLength = length(waterLightLoopData.EndPos - waterLightLoopData.StartPos);  
    waterLightLoopData.RayDirection = (waterLightLoopData.EndPos - waterLightLoopData.StartPos);

    float ScaleFactor = min(waterLightLoopData.RayLength, _MaxDistance) / (waterLightLoopData.RayLength + 1e-8);    
    waterLightLoopData.NormalizedRayDirection = (waterLightLoopData.EndPos - waterLightLoopData.StartPos) * ScaleFactor;
    waterLightLoopData.NormalizedRayLength = waterLightLoopData.RayLength * ScaleFactor;
    waterLightLoopData.NormalizedAmbientDepth = abs(safeNormalize(waterLightLoopData.EndPos).y) * waterLightLoopData.NormalizedRayLength;
    waterLightLoopData.NormalizedSunDepth = waterLightLoopData.NormalizedAmbientDepth / abs(safeNormalize(V).y);
    waterLightLoopData.Dither = dither;

    waterLightLoopData.WaterAbsorption = _AbsorptionC.rgb * _AbsorptionStrength;
    waterLightLoopData.ScatterCoefficient = _ScatterC.rgb * _ScatterStrength;
...
#undef EVALUATE_BSDF_ENV_SKY
    ...
    if (featureFlags & LIGHTFEATUREFLAGS_DIRECTIONAL)
    {
        for (i = 0; i < _DirectionalLightCount; ++i)
        {
            ...
            {
                DirectLighting lighting = WaterEvaluateBSDF_Directional(waterLightLoopData,featureFlags,context, V, posInput, preLightData, _DirectionalLightDatas[i], bsdfData, builtinData);
                AccumulateDirectLighting(lighting, aggregateLighting);
            }
        }
    }
    ...
    WaterPostEvaluateBSDF(waterLightLoopData,context, V, posInput, preLightData, bsdfData, builtinData, aggregateLighting, lightLoopOutput);
