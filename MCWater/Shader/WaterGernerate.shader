Shader "Shader Graphs/McWater"
{
    Properties
    {
        [NoScaleOffset]_Texture2D("Texture2D", 2D) = "white" {}
        _Size("Size", Float) = 0
        _largeWaveScale("largeWaveScale", Float) = 0
        _nomralDistance("nomralDistance", Float) = 0
        _normalDistanceSoftness("normalDistanceSoftness", Float) = 0
        _AbsorptionC("AbsorptionC", Color) = (0, 0, 0, 0)
        _ScatterC("ScatterC", Color) = (0, 0, 0, 0)
        _AmbientLC("AmbientLC", Color) = (0, 0, 0, 0)
        _DirectLC("DirectLC", Color) = (0, 0, 0, 0)
        _ShadowDitherDepth("ShadowDitherDepth", Float) = 0
        _AbsorptionStrength("AbsorptionStrength", Float) = 0
        _ScatterStrength("ScatterStrength", Float) = 0
        _SampleCount("SampleCount", Float) = 0
        _Float("Float", Float) = 0
        _MaxDistance("MaxDistance", Float) = 0
        _PhaseG("PhaseG", Float) = 0
    ...
    SubShader
    {
        ...
        float _Size;
        float _largeWaveScale;
        float _nomralDistance;
        float _normalDistanceSoftness;
        float4 _AbsorptionC;
        float4 _ScatterC;
        float4 _AmbientLC;
        float4 _DirectLC;
        float _ShadowDitherDepth;
        ...
        {
            SurfaceDescription surface = (SurfaceDescription)0;
            float3 _HDSceneColor_e43b1e7a18c2428ead3180a0a01b567c_Output_2_Vector3 = Unity_HDRP_SampleSceneColor_float(float4(IN.NDCPosition.xy, 0, 0).xy, 0, 1.0);
            float _Property_80998f6284de45f28f4d2a2f6c03db8b_Out_0_Float = _Float;
            float3 _Multiply_fc38bdf370c84dbab61d6e986dabbc69_Out_2_Vector3;
            Unity_Multiply_float3_float3(_HDSceneColor_e43b1e7a18c2428ead3180a0a01b567c_Output_2_Vector3, (_Property_80998f6284de45f28f4d2a2f6c03db8b_Out_0_Float.xxx), _Multiply_fc38bdf370c84dbab61d6e986dabbc69_Out_2_Vector3);
            float3 _EmissionNode_0df9498429de40a7a9e406def14e89a2_Output_0_Vector3 = Unity_HDRP_GetEmissionHDRColor_float(_Multiply_fc38bdf370c84dbab61d6e986dabbc69_Out_2_Vector3.xyz, 1, 0.001);
            surface.BaseColor = IsGammaSpace() ? float3(0, 0, 0) : SRGBToLinear(float3(0, 0, 0));
            surface.Emission = _EmissionNode_0df9498429de40a7a9e406def14e89a2_Output_0_Vector3;
            surface.Alpha = 1;
            surface.BentNormal = IN.TangentSpaceNormal;
            surface.Smoothness = 0.98;
            surface.Occlusion = 1;
        ...
        void BuildSurfaceData(FragInputs fragInputs, inout SurfaceDescription surfaceDescription, float3 V, PositionInputs posInput, out SurfaceData surfaceData, out float3 bentNormalWS)
        {
            ...
            #include "WaterShaderPassForward.hlsl"
